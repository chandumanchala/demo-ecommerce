...
app.post("/signup", async (req, res) => {
  ...
  if (check) {
    return res.status(400).json({
      success: false,
      errors: "Exixting User Found with same email address",
    });
  }
  let cart = {};
  for (let i = 0; i < 300; i++) {
...
const fetchUser = async (req, res, next) => {
  ...
  if (!token) {
    res.sttus(401).send({ errors: "Please authenticate using valid token " });
  } else {
    try {
...
//creating a endpoint for removing a product fro cartdata
